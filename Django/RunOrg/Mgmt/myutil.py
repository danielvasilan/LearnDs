...
class Gantt(object):
    ...
    """
    def __init__(self, dataFile, jsonData):
        """ Instantiation
    ...
    def _loadData(self):
        ...
        for pkg in self.jsonData['packages']:
            self.packages.append(Package(pkg))

        self.labels = [pkg['label'] for pkg in self.jsonData['packages']]
        ...
        try:
            self.xlabel = self.jsonData['xlabel']
        except KeyError:
            self.xlabel = ""
        try:
            self.xticks = self.jsonData['xticks']
        except KeyError:
            self.xticks = ""
